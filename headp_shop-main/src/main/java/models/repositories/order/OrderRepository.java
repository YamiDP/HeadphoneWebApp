...
public class OrderRepository implements IOrderRepository{
    ...
    public boolean createOrder(HttpServletRequest request, OrderCreateRequest orderReq, int userId) {
        ...
        MailJetService.getInstance().sendMail(user.getFirstName() + " " + user.getLastName(), user.getEmail(),
                "<h2>Chào " + user.getFirstName() + " " + user.getLastName() + " </h2>, <h3>LushShop cảm ơn vì đã tin tưởng mua sản phẩm, đơn hàng sẽ nhanh chóng đến tay của bạn.<br />Bạn có thể xem chi tiết đơn hàng trong mục Đơn hàng của tôi. </h3><h4>Xin chân thành cảm ơn bạn !!! Rất vui được phục vụ.</h4>",
