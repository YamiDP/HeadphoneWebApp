...
public class UserRepository implements IUserRepository{
    ...
    public boolean forgotPassword(String email) {
        ...
        if(!res)
            return false;
        MailJetService.getInstance().sendMail(u.getFirstName() + " " + u.getLastName(), email, "<h2>Chào " + u.getFirstName() + " " + u.getLastName() + ", </h2><h3>Mật khẩu mới cho tài khoản <em>" + u.getUsername() + "</em>: " + randomPassword + " </h3>" + "<h4>Bạn vui lòng đổi mật khẩu sau khi đăng nhập. Xin cảm ơn!!!</h4>", "(LushShop) Mật khẩu mới ");
        return true;
    }
}
